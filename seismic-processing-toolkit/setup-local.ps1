...
Set-Location $root
try { $version = node --version } catch { Write-Error "Node.js 22.12.0 or newer is required. Install Node 24 LTS, then rerun this script."; exit 1 }
...
try { npm --version | Out-Null } catch { Write-Error "npm 10 or newer is required for source-checkout setup."; exit 1 }
npm ci
exit $LASTEXITCODE
