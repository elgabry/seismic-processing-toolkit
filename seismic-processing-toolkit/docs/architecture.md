...
The local-distribution layer remains outside the browser processing architecture. `scripts/serve-local.mjs` is a dependency-free Node static server with a configurable root and loopback default; it serves only built assets and a status-only `/healthz` endpoint. `package-local-release.mjs` copies the Vite output, server, launchers, metadata, checksums, SmartSolo worker chunk, and legacy static viewer into a self-contained directory. It contains no source, package manager dependencies, processing service, or local-file upload path.
