...
Publication sections follow the same boundary: `visualization/section` owns serializable display options, layout, amplitude statistics/mapping, density rasterization, optional wiggle overlay, and axes. `export/png/publication-section-export.ts` creates a new canvas through `PngExportService`; `PngExportDialog` only gathers public trace/header values into the render model. Rendering conditions pixels for display and never changes a `SegyDataset`, trace cache, or interactive viewport.
