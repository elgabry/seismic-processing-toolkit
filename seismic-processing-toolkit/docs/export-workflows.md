...
Both CSV and PNG downloads are browser-local in source, preview, and packaged local-server modes. The local server has no export endpoint and does not receive generated bytes.
