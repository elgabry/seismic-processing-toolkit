...
**Publication grayscale section** is an additional PNG target for a selected visible section, explicit trace range, full dataset, or a selected gather key. It rasterizes samples directly into a fresh portrait canvas, with time increasing downward. The reference-style preset is 1200 × 2400 px, white page, black frame, editable two-line title, receiver axis, optional time axis, positive-black polarity, gray 128 at zero amplitude, global 99th-percentile clipping, gamma 0.80, no grid, and no wiggle overlay.

The density convention is symmetric: clipped positive values become black and clipped negative values become white. `positive-white` reverses this. `None`, global RMS, bounded-histogram global percentile, trace RMS, and the existing gain-module AGC are display-only choices; the underlying SEG-Y samples and active viewport are unchanged. Non-finite/missing values are neutral gray and are counted as render diagnostics. Receiver numbers/stations can retain nonuniform spacing; choose equal spacing when that better represents the section. Invalid or duplicate station values fall back safely and should be treated as a display warning.

The rasterizer antialiases vertical decimation, uses typed arrays, avoids sample objects, and uses only a small cache of temporary AGC traces. Pixel limits are checked before allocation; use a smaller preset instead of expecting silent downscaling. Reference-like contrast depends on the selected gather and actual processing. Apply bandpass, correlation, AGC, gain, or deconvolution in the processing graph when those operations must be part of the documented seismic result.
