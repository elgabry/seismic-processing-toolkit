...
The local static server reads only small immutable application assets from the release directory. It never receives browser `File` objects, sample arrays, worker buffers, CSV content, or PNG content; those remain under browser ownership. Packaging copies built chunks rather than decoding or duplicating seismic data.
